...
public class OclConfig {
	...
	/** Local address for TCP telemetry/vision listeners. Loopback is the safe default; use 0.0.0.0 deliberately for remote controllers. */
	...
	/** Explicit path to the FFmpeg binary used by virtual cameras. Blank = search the system PATH. */
	...
	private static OclConfig load() {
		...
		return new OclConfig().normalize();
	}

	/** Normalize values from hand-edited JSON before any runtime component consumes them. */
	public OclConfig normalize() {
		if (transport == null) transport = Transport.UDS;
		if (tcpUrl == null || tcpUrl.isBlank()) tcpUrl = "tcp://127.0.0.1";
		if (tcpBind == null || tcpBind.isBlank()) tcpBind = "127.0.0.1";
		if (udsDir == null) udsDir = "";
		if (ffmpegPath == null) ffmpegPath = "";
		inputStalenessTicks = clamp(inputStalenessTicks, 1, 20);
		cameraWidth = clamp(cameraWidth, 16, 1920);
		cameraHeight = clamp(cameraHeight, 16, 1080);
		return this;
	}
	...
	/**
	 * Resolve the TCP endpoints from this config. The mod binds telemetry and vision on the configured
	...
	public LinkConfig.TcpEndpoints toEndpoints() {
		...
		return new LinkConfig.TcpEndpoints(
				host, tcpBind.trim(), LinkConfig.TELEMETRY_PORT, LinkConfig.INSTRUCTION_PORT, LinkConfig.VISION_PORT);
	}
	...
	private static String hostOf(String tcpUrl) {
		...
		if (s.startsWith("[")) {
			int close = s.indexOf(']');
			if (close > 1) return s.substring(1, close);
		}
		int firstColon = s.indexOf(':');
		int lastColon = s.lastIndexOf(':');
		if (firstColon >= 0 && firstColon == lastColon) s = s.substring(0, firstColon);
