 ...
 *   rgb     : vis_w*vis_h*3 x u8      row-major, top-left origin, interleaved RGB, normalized /255
 *   depth   : vis_w*vis_h   x u16-LE  row-major, top-left origin, distance normalized /65535
...
public final class BinaryCodec {
	 ...
	 * ({@code w*h*3} unsigned bytes) is written first, then the depth plane ({@code w*h} unsigned
	 * 16-bit values); both are row-major, top-left origin, normalized 0..1. The packed precision is the
	 * same representation used by the recorder and cuts the default frame from ~5.1 MiB to ~1.6 MiB.
	 */
	public static byte[] encodeVision(VisionFrame f) {
		...
		float[] rgb = f.rgb();
		for (int i = 0; i < rgbLen; i++) buf.put((byte)Math.round(clamp01(rgb[i]) * 255.0f));
		float[] depth = f.depth();
		for (int i = 0; i < depthLen; i++) buf.putShort((short)Math.round(clamp01(depth[i]) * 65535.0f));
		return buf.array();
	}
