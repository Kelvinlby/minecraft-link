 ...
 *
 * <p>Roles mirror {@link UdsBridge}: the mod <b>binds</b> telemetry and vision on the configured local
 * address and the controller connects; the mod <b>connects</b> to the controller's instruction server.
 */
public final class TcpBridge extends AbstractLinkBridge {
	...
	public synchronized void start(LinkConfig.TcpEndpoints endpoints) {
		...
		OpenCrafterLink.LOGGER.info(
				...
				endpoints.bindHost(), endpoints.telemetryPort(), endpoints.host(), endpoints.instructionPort(),
				endpoints.bindHost(), endpoints.visionPort());
	}
	...
	protected ServerSocketChannel openTelemetryServer() throws IOException {
		return bind(endpoints.bindHost(), endpoints.telemetryPort());
	}

	@Override
	protected ServerSocketChannel openVisionServer() throws IOException {
		return bind(endpoints.bindHost(), endpoints.visionPort());
	}
	...
	/** Bind a fresh TCP server socket on the configured address, allowing quick rebinds. */
	private static ServerSocketChannel bind(String host, int port) throws IOException {
