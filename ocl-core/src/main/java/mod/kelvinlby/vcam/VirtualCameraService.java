 ...
 *
 * <h2>Frame source</h2>
 * Frames come from {@link VisionFrameBus} — the same converted {@link VisionFrame}s the recorder consumes,
...
public final class VirtualCameraService {
	...
	private void applyDesiredState() {
		...
		if (rgbSink != null || depthSink != null) {
			VisionFrameBus.setActive(FRAME_CONSUMER, true);
			startClock();
		} else {
			stopClock();
		...
		if (needRgb && next < free.size()) {
			...
			if (rgbSink == null) {
				lastError = "FFmpeg could not start the RGB virtual camera — see the log and the "
						+ "FFmpeg path in Open Crafter Link's Sensors settings.";
