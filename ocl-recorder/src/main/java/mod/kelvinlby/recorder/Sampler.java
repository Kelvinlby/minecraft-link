 ...
 * the newest RGBD frame ({@link VisionFrameBus#latest()}) together with the current action set
...
public final class Sampler {
	...
	/** First per-sample persistence failure; closing a stream later may otherwise appear successful. */
	...
	public synchronized SaveResult stop(ProgressListener listener) {
		...
		String closeError = writer.close(written.get(), dropped.get(), repeated.get());
		String error = writeError.get() != null ? writeError.get() : closeError;
	...
	private void clockLoop() {
		...
		while (running) {
			...
			VisionFrame fresh = VisionFrameBus.latest();
	 ...
	 * once; frames arrive here in sample order on {@link VisionFrameBus#takeEager} and are paired back up
	...
	private void eagerClockLoop() {
		long lastSeq = -1L;
		while (running) {
			VisionFrameBus.SeqFrame fresh;
			try {
				fresh = VisionFrameBus.takeEager(100L);
	...
	private void persist(Sample s) {
		try {
			...
			writeError.compareAndSet(null, "failed to write sample " + s.seqno() + ": "
