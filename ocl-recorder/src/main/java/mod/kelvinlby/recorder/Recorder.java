...
import mod.kelvinlby.link.VisionFrameBus;
 ...
 * The RGBD frames come from the link's existing vision pipeline via {@link VisionFrameBus}, which a
 * session enables on start and disables on stop so the bridges skip the tap when nobody is recording.
 */
public final class Recorder {
	private static final Object FRAME_CONSUMER = new Object();

	private static final DateTimeFormatter STAMP = DateTimeFormatter.ofPattern("yyyy-MM-dd_HH-mm-ss");
	private static final Path REPLAY_INBOX = OpenCrafterLink.profileDir().resolve("replay");
	...
	/** Live input polling is needed only for a running human demonstration, never while idle/replaying. */
	public synchronized boolean shouldReadLiveActions() {
		return running && replay == null;
	}
	...
	private void start() {
		...
		VisionFrameBus.clearSequenced();
		VisionFrameBus.setActive(FRAME_CONSUMER, true); // bridges start publishing converted frames
		...
		try {
			...
			VisionFrameBus.setActive(FRAME_CONSUMER, false);
			VisionFrameBus.clearSequenced();
	...
	private void stopAsync() {
		...
		Thread t = new Thread(() -> {
			...
			if (archive && result != null && result.faithful()) {
	...
	private static Path sessionDir() {
		Path root = OpenCrafterLink.profileDir().resolve("recording");
